...
import numpy as np
import pandas as pd
from data_utils import load_data, DATA_MAP,my_load_data_copy
...
def get_metrics(args, only_raw = False, only_normalized = False, only_ordinal = False):
    X_train, X_test, y_train, y_test = my_load_data_copy(args)
...
def get_metrics_20news(args, only_raw = False, only_normalized = False, only_ordinal = False):
    X_train, X_test, y_train, y_test = load_data(args)
    if isinstance(y_test, pd.Series):
        y_test = np.array(y_test)
    #y_test = y_test.to_numpy()
    my = str(int(args.split_idx) + 1)
    if args.exp_dir is None:
        args.exp_dir = Path('exp') / args.dataset / args.setting / "split{}".format(args.n_splits) / "split{}".format(my)
    score_dir = args.exp_dir / 'scores'
    if not os.path.exists(score_dir):
        raise ValueError("Score directory {} does not exist".format(score_dir))
